...
void main() {
  group('Memory Optimization Tests', () {
    ...
    test('Memory optimization should reduce initial service count', () {
      ...
      const optimizedCoreServiceCount =
          6; // Core services in MemoryOptimizedBinding

      expect(optimizedCoreServiceCount, lessThan(originalServiceCount));

      final reductionPercentage =
          ((originalServiceCount - optimizedCoreServiceCount) /
                  originalServiceCount) *
              100;
      ...
      print(
          '📊 Estimated reduction: ${reductionPercentage.toStringAsFixed(1)}%');
    });
  });
}
...
void runMemoryUsageTest() {
  ...
  print(
      '📉 Memory reduction: ${reductionMB}MB (${reductionPercent.toStringAsFixed(1)}%)');
