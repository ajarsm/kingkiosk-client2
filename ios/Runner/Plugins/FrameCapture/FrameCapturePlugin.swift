...
public class FrameCapturePlugin: NSObject, FlutterPlugin {
  // Metal resources for enhanced texture operations
  ...
  private func initializeMetal() {
    ...
    if device != nil {
      print("✅ FrameCapturePlugin: Metal device initialized successfully")
    } else {
      print("❌ FrameCapturePlugin: Failed to initialize Metal device")
    }
  }
  ...
  private func handleCaptureFrame(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    print("🎥 FrameCapturePlugin: Legacy capture from renderer \(rendererId) (\(width)x\(height))")
  ...
  private func getWebRTCMetalTexture(rendererId: Int) -> MTLTexture? {
    // Enhanced WebRTC Metal texture access for iOS
  ...
  private func createSimulatedWebRTCTexture(device: MTLDevice, textureId: Int64, rendererId: Int) -> MTLTexture? {
    ...
    let frameCounter = Int(frameTime * 30) % 1000 // 30 FPS with longer cycle
  ...
  // MARK: - Enhanced Metal Texture Capture
  
  private func captureFrameFromTexture(rendererId: Int, width: Int, height: Int) -> FlutterStandardTypedData? {
    print("🎥 FrameCapturePlugin: Legacy capture method - redirecting to enhanced capture")
    
    // Try to get the real WebRTC Metal texture
    if let webrtcTexture = getWebRTCMetalTexture(rendererId: rendererId) {
      if let frameData = captureFromRealMetalTexture(texture: webrtcTexture, width: width, height: height) {
        print("✅ Successfully captured real WebRTC frame: \(frameData.data.count) bytes")
        return frameData
      }
    }
    
    print("⚠️ Real WebRTC texture access not available - using enhanced fallback")
    return createEnhancedFallbackFrame(width: width, height: height, rendererId: rendererId)
  }
  ...
  // Legacy support method - maintained for backward compatibility
  private func getRendererTextureId(renderer: Any) -> Int {
    return extractTextureIdFromRenderer(renderer: renderer)
  }
