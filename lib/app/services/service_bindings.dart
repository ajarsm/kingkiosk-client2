...
class ServiceBindings {
  ...
  static void setupMqttAutoConnect() {
    try {
      ...
      if (mqttEnabled) {
        // Get MQTT settings
        final brokerUrl =
            storageService.read<String>(AppConstants.keyMqttBrokerUrl) ??
                'broker.emqx.io';
        final brokerPort =
            storageService.read<int>(AppConstants.keyMqttBrokerPort) ?? 1883;
  ...
  static void _notifySettingsControllersOfMqttConnection() {
    try {
      // Try to find and update SettingsControllerFixed (the only active settings controller)
